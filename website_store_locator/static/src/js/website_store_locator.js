odoo.define('website_store_locator.website_store_locator', function (require) {
    ...
    $(function () {
        ...
        ajax.loadXML('/website_store_locator/static/src/xml/base.xml', qweb).then(() => {
            ...
            if (show_locator === 'yes') {
                ...
                $('#search-store-by-categ').on('change', function () {
                    var id = $(this).val();
                    var count = 0;
                    bounds = new google.maps.LatLngBounds();
                    $.each(initialize_data.map_stores_data, function (key, value) {
                        if (id == value.category_id || id == 'all') {
                ...
                $('#search-store-by-country').on('change', function () {
                    var id = $(this).val();
                    var count = 0;
                    bounds = new google.maps.LatLngBounds();
                    $.each(initialize_data.map_stores_data, function (key, value) {
                        if (id == value.country_id || id == 'select') {
                            show_marker_and_shop(parseInt(key));
                            count++;
                        } else {
                            hide_marker_and_shop(parseInt(key));
            ...
            function load_shop_list(key, store, info) {
                var html = qweb.render('SaleShopStoreBox', {
                    key: key,
                    store: store,
                    info: info,
                })
                $('.store-menu ul').append(html);
