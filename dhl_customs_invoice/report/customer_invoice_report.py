...
class DhlCustomerInvoice(models.AbstractModel):
    ...
    def get_lines_data(self, inv, delivery):
        ...
        for deliv in delivery.pack_operation_product_ids:
            same_product_lines = delivery.pack_operation_product_ids.filtered(lambda p: p.product_id.id == deliv.product_id.id)
            total_qty_done = sum([s.qty_done for s in same_product_lines])
            if not any(d['id'] == deliv.product_id.id for d in line_list):
                line = inv.order_line.filtered(lambda l: l.product_id.id == deliv.product_id.id)
                if line:
        ...
        product_list = [dimension_tmpl_id.product_variant_id.id, weight_tmpl_id.product_variant_id.id]
        for line_id in inv.order_line:
            if line_id.product_id.id in product_list:
                ...
                line_list.append({
                    ...
                    'country_of_origin': line_id.product_id.x_Country_of_origin.name,
