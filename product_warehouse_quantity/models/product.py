...
class ProductTemplate(models.Model):
    ...
    def _get_warehouse_quantity(self):
        for record in self:
            ...
            if product_id:
                quant_ids = self.env['stock.quant'].sudo().search([('product_id','=',product_id[0].id),('location_id.usage','=','internal'),('company_id', '=', self.env.user.company_id.id)])
                t_warehouses = {}
                ...
                for location in t_warehouses:
                    ...
                    while (not warehouse and location1):
                        warehouse_id = self.env['stock.warehouse'].sudo().search([('lot_stock_id','=',location1.id),('company_id', '=', self.env.user.company_id.id)])
